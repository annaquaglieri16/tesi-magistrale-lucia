...
cambio_si_no <- risposte %>%
  ...
  # professione
  mutate(stato_prof_durante = tolower(stato_prof_durante)) %>%
  left_join(professione, by = c("stato_prof_durante"="Nomi_iniziali")) %>%
  dplyr::select(-stato_prof_durante) %>%
  dplyr::rename(stato_prof_durante = nomi_nuovi)
...
```{r}
#ggplot(risposte, aes(x = stato_prof_durante)) + geom_bar() + theme_bw() + coord_flip()
```

* Sembra che lo stato professionale contenga ancora molte categorie

```{r}
prop.table(table(cambio_si_no$stato_prof_durante))
```


```{r}
prop.table(table(cambio_si_no$tipo_abitazione))
```
